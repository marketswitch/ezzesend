...
class CampaignController extends Controller
{
    ...
    public function createCampaign(Request $request)
    {
        ...
        $contactTags      = ContactTag::where('user_id', $user->id)
            ->with('contacts')
            ->orderBy('name', 'asc')
            ->get();
        ...
        $whatsappAccounts = WhatsappAccount::where('user_id', $user->id)
            ->with('templates')
            ->get();

        return view('Template::user.campaign.create', compact(
            'pageTitle',
            'contactLists',
            'templates',
            'whatsappAccounts',
    ...
    public function saveCampaign(Request $request)
    {
        ...
        $user = getParentUser();
        \Log::info('Segment:', ['segment' => $request->segment]);
        ...
        $campaignExists = Campaign::where('user_id', $user->id)
        ...
        CampaignContact::where('campaign_id', $campaign->id)->update([
            'send_at' => $sendAt,
            'is_trigger' => Status::NO,
            'is_failed' => Status::NO,
            'error_message' => null,
        ]);
    ...
    public function report($id)
    {
        ...
        $widget['total_sent'] = Message::where('campaign_id', $campaign->id)->count();
        ...
        return view('Template::user.campaign.report', compact(
            'pageTitle',
            'campaign',
            'widget',
            'campaignContacts',
            'messages',
            'lastCampaignMessageCount'
        ));
    }
    ...
    public function downloadCsv($campaign)
    {
        ...
        $rows = [
            [
                ...
                $campaign->total_send,
                $campaign->total_success,
                $campaign->total_failed,
                showDateTime($campaign->send_at)
            ],
        ];
